...
import { fetchFlowById, cleanFlowForExport } from '$lib/api/appmixer.js';
import {
  getGitHubConfig,
  createBranch,
  createOrUpdateFile,
  ...
  try {
    ...
    for (const flowInfo of flows) {
      ...
      try {
        ...
        // Clean the flow for export (name normalized to the E2E identity)
        const cleanedFlow = cleanFlowForExport(fullFlow, name);
