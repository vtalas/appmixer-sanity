...
| `/releases`                        | Release — connector versions on appmixer-components `master` (production) against appmixer-connectors `dev`: what needs releasing (new / major / minor / patch), unbumped changes, and a **Release** button (admins) that commits each selected connector to `master` as `<connector> <version> (<type>)` |
...
| `RELEASE_TARGET_REPO` / `RELEASE_TARGET_BRANCH`                 | Release page target, defaults `Appmixer-ai/appmixer-components@master` — the GitHub token needs push access here |
