...
**Filtering.** The instance is shared, so the unfiltered widget lists every template anyone published. The SDK widget does not read the hub's tabs from `/automation-hub/settings` — the page narrows it itself: `options.flows.templates.header.categories = { visible: true, tabs: [{ category: <categoryId>, label }] }` offers only that category next to "All", and the `categoryIds` state preselects it (the templates query filters on the *selected* categories, so without it the list starts on "All"). "All" still lists everything. "My automations" (instances) is per-user and the widget has no category filter for it — only search and `onlyRunning` — and the configured user (`test@appmixer.ai`) is shared, so other people's test instances (two "Youtube Automation", a "QA Flow - May12") showed up next to the CI responders. The page narrows the *queries* instead: the widget option `api` overrides `getFlows` and `getFlowsCount` (`hubApi()`), and a query whose filter has `type:integration-instance` gets one `templateId:<id>` clause per template of the category — the engine's filter parser turns a repeated key into `$in` (verified on dev-automated-00001: 3 instances, count 3). A category with no templates hides all instances; no category (another instance) means the unfiltered hub, instances included. Options are deep-merged (`deepmerge`) over the widget defaults, so a partial `options` object is fine.
...
**Editing a template in the embedded Designer.** Edit opens the template's draft (`originFlowId`) in `appmixer.ui.Designer` with `integrationTemplateMode: true` — the same widget and mode Studio's `/integration-designer/<id>` route uses. Its header has Wizard Builder ("Preview & Adjust"), Create/Start Test and Publish instead of Start/Stop. Publish updates the existing template the way `publish-integration.js` does (template component IDs via `componentIdMap`, `revision` + 1); **edit the draft, never the template** — opened on the template, Publish finds no template whose `originFlowId` is it and clones a new one. The Designer mounts once into `#template-designer`, a fixed full-screen overlay at `z-index: 100`, below the SDK's own modals (101–103), so the test Wizard opens on top; later edits reuse it (`set('componentId', null)`, `set('flowId', id)`, `open()`). Its ← emits `close`: the page runs the default (`event.next()`, unmount), hides the overlay and `invalidateAll()`s for the new revision. The hub itself is not reloaded (see the paging loop above). Events without an SDK default are handled like Studio does: `integration-test:create` clones the draft to an `integration-test` flow and opens the Wizard on it, then calls `next`; `integration-test:edit` opens the Wizard; `integration-test:insights` opens Studio's `/insights/logs/<id>`. The menu leaves out `flow:clone`, `flow:insights`, `flow:turn-into-automation` (no default) and `flow:remove` (would delete the draft; its `flow:remove-after` default also calls an undefined `goToFlows`). Cleanup on leaving the page is a `$effect` teardown, not `onDestroy` — `onDestroy` also runs in SSR, where `document` is undefined (500).
