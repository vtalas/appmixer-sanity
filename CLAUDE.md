...
- **Token** — the caller's GitHub token (Settings) or `SANITY_GITHUB_TOKEN`: read access to both repos (appmixer-components is private), push access to the head repo; the PR is opened in the target repo. Commits and the PR are authored by the token's owner.
...
Environment: `RELEASE_SOURCE_REPO` / `RELEASE_SOURCE_BRANCH` (default `Appmixer-ai/appmixer-connectors` / `dev`), `RELEASE_TARGET_REPO` / `RELEASE_TARGET_BRANCH` (PR base, default `Appmixer-ai/appmixer-components` / `master`), `RELEASE_HEAD_REPO` / `RELEASE_HEAD_BRANCH` (PR head, default `vtalas/appmixer-components` / `release`).
